...
/* ── Floating stat pill ──────────────────────────────────── */
function StatPill({ icon, label, value, style }) {
  return (
    <div style={{
...
function LoginPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    const authUser = MOCK_AUTH_USERS[email.toLowerCase()];
    if (authUser && pw === 'password') {
      login(authUser);
  ...
  const inputStyle = (name) => ({
    width: '100%', background: focused === name ? 'rgba(99,102,241,0.06)' : 'rgba(15,23,42,0.6)',
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', background: '#020617', fontFamily: "'Inter', sans-serif" }}>
      ...
      <div style={{
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12, alignItems: 'flex-start', textAlign: 'left' }}>
            {[
              ...
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                <span style={{ color: '#6366f1', fontSize: 16, lineHeight: 1 }}>{f.icon}</span>
        ...
        {/* Floating stat pills */}
        <StatPill icon="👥" label="Active employees" value="1,284" style={{ top: '12%', right: '8%', animation: 'floatA 5s ease-in-out infinite' }} />
        <StatPill icon="✅" label="Leaves approved" value="98.2%" style={{ bottom: '15%', left: '6%', animation: 'floatB 6s ease-in-out infinite' }} />
        <StatPill icon="⚡" label="Avg. response time" value="4 min" style={{ bottom: '30%', right: '5%', animation: 'floatA 7s ease-in-out infinite 1s' }} />
      </div>
        ...
        <div style={{ width: '100%', maxWidth: 360 }} className="animate-fade-in">
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
            ...
            <div style={{ marginBottom: 16 }}>
              <label style={{ fontSize: 12, fontWeight: 500, color: '#94a3b8', display: 'block', marginBottom: 7 }}>
                Work email
              </label>
              <input
                type="email"
            ...
            <div style={{ marginBottom: 8 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 7 }}>
                ...
                <span style={{ fontSize: 11, color: '#4f46e5', cursor: 'pointer' }}>Forgot password?</span>
              </div>
            ...
            {error && (
              <div style={{
                ...
                <SvgAlert size={14} style={{ color: '#f87171', flexShrink: 0 }} />
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {[
              { label: 'HR Admin', sub: 'admin@sentient.dev', color: '#6366f1', badge: 'Full access' },
              { label: 'Manager',  sub: 'manager@sentient.dev', color: '#8b5cf6', badge: 'Team view' },
              { label: 'Employee', sub: 'employee@sentient.dev', color: '#06b6d4', badge: 'Self-service' },
            ].map(d => (
              <button key={d.sub} onClick={() => fillDemo(d.sub, 'password')} style={{
              ...
              onMouseLeave={e => { e.currentTarget.style.background = `${d.color}0d`; e.currentTarget.style.borderColor = `${d.color}28`; }}>
                <div style={{
                  ...
                  background: `${d.color}22`, display: 'flex', alignItems: 'center', justifyContent: 'center',
                  color: d.color, fontSize: 12, fontWeight: 700, flexShrink: 0,
                }}>
                  {d.label.charAt(0)}
                </div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: 13, fontWeight: 600, color: '#cbd5e1' }}>{d.label}</div>
                  <div style={{ fontSize: 11, color: '#475569', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{d.sub}</div>
                </div>
            ...
            <p style={{ fontSize: 11, color: '#1e293b', marginTop: 4, textAlign: 'center' }}>
              All demo accounts use password: <code style={{ color: '#475569', background: 'rgba(71,85,105,0.15)', padding: '1px 5px', borderRadius: 4 }}>password</code>
            </p>
          </div>
        </div>
