...
const FIRST_NAMES = [
  "James","Oliver","William","Benjamin","Lucas","Henry","Alexander","Mason","Ethan","Daniel",
  "Michael","Matthew","Jack","Logan","Owen","Noah","Liam","Aiden","Carter","Ryan",
...
function resolveMaritalStatus(idx: number): MaritalStatus {
  const r = idx % 4;
  if (r === 0) return MaritalStatus.SINGLE;
  if (r === 1) return MaritalStatus.MARRIED;
  if (r === 2) return MaritalStatus.MARRIED;
...
function getName(idx: number, part: "first" | "last"): string {
  const arr = part === "first" ? FIRST_NAMES : LAST_NAMES;
...
async function seedBulkEmployees(f: FoundationMaps): Promise<BulkEmployee[]> {
  ...
  for (const bc of BU_CONFIGS) {
    ...
    for (const slot of ordered) {
      ...
      const firstName = getName(globalIdx, "first");
      const lastName  = getName(globalIdx + 17, "last");
      const email     = `${firstName.toLowerCase()}.${lastName.toLowerCase()}${globalIdx}@sentient.dev`;
      ...
      const emp = await prisma.employee.create({
        data: {
          ...
          lastName,
          email,
          phone: `+1-555-${String(globalIdx).padStart(4, "0")}`,
          hireDate,
