...
export class SupervisorGateService {
  ...
  /**
   * WHY split out from shouldRouteToAnalyticsSql: routeReason() and the runner's
   * route trace need to explain WHICH condition closed the gate (flag off vs.
   * wrong role vs. mixed operational turn), not just that it did. It is
   * recomputed rather than threaded through graph state because the repeat calls
   * only happen on an analytical turn with debug logs on — two cheap boolean
   * checks and one array filter — and a state channel for a value that exists
   * purely to explain a decision would outlive its usefulness.
   *
   * WHY public: the runner owns the graph's route trace and needs these
   * diagnostics, but the gate owns the decision they explain.
   */
  analyticsSqlGateDiagnostics(
    classification: SupervisorIntentClassification,
    actor: AiActorContext,
