...
describe('agent state-machine routing fixtures', () => {
  ...
  it('routes leave questions to Leave Agent', () => {
    const route = classifier.classify('What is my leave balance and last leave?');

    expect(route.requiredAgents).toEqual([AgentType.LEAVE_AGENT]);
  });

  it('routes manager multi-domain questions to leave, OKR, and analytics specialists', () => {
    const route = classifier.classify('Summarize leave coverage, OKR risk, and dashboard trends for my team.');
  ...
  it('routes ambiguous prompts to clarification', () => {
    const route = classifier.classify('Can you help me with my objective?');
