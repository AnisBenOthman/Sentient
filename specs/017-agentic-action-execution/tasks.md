...
> **⚠️ ACTION REQUIRED BEFORE RUNNING HR CORE LOCALLY.** `EmployeesService.defaultInclude()` now selects `businessUnit.country`, so **every employee read fails at runtime** until this migration is applied. Unit tests do not catch it — Prisma is mocked. `prisma migrate status` in `apps/hr-core` shows **four** unapplied migrations, three of which predate this feature: `20260521180000_add_employee_gender`, `20260612000000_add_business_unit_currency`, `20260729000000_ai_analytics_views`, plus `20260731000000_add_business_unit_country`. `apps/ai-agentic` likewise has `20260729000100_add_agent_task_log_generated_sql` pending alongside this feature's migration. Applying these was deliberately left to the operator — the pre-existing drift means `migrate dev` could do more than this feature intends. Seeding actual ISO country values for existing business units is also still outstanding.
