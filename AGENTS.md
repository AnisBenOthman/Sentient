...
## Recent Changes
- seed-200-employees: Full DB reset + reseed with 200 realistic employees (HQ=70, France=50, UAE=45, UK=35), 3-year salary history, skill evolution, leave balances/requests, 5 performance review cycles (4 CLOSED all COMPLETED, 1 ACTIVE 60% SUBMITTED/40% PENDING), position-skill requirements (50 pairs for radar gap chart), all 4 SkillDomain values populated, employment status mix (ACTIVE=176, TERMINATED=12, RESIGNED=8, ON_LEAVE=3, PROBATION=1), 4 demo accounts; run via `node_modules/.bin/ts-node --transpile-only --compiler-options '{"module":"CommonJS"}' prisma/seed.ts` from `apps/hr-core`
